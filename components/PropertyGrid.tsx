  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
      {properties.map((property) => (
        <div 
          key={property.id} 
          className={`bg-white rounded-3xl overflow-hidden shadow-[0_2px_10px_rgba(0,0,0,0.02)] border transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(0,0,0,0.04)] ${property.active ? 'border-blue-500 ring-1 ring-blue-500' : 'border-slate-100/50'}`}
        >
          <div className="relative h-[220px] w-full p-3.5">
            <div className="relative w-full h-full rounded-2xl overflow-hidden">
              ...
              />
              <div className="absolute top-4 left-4 bg-white/95 backdrop-blur-sm px-4 py-1.5 rounded-full text-[13px] font-medium text-slate-700">
                {property.type}
              </div>
            </div>
          </div>
          ...
          <div className="p-5 pt-2">
            <div className="flex justify-between items-start mb-3">
              ...
              <button className={`p-2.5 rounded-full transition-colors ${property.active ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}>
